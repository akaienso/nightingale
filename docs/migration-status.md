...
### ⚠️ Open: the Yarn version is unpinned, and `--immutable` fails

`yarn install --immutable` — what CI runs by default — is rejected:

```
➤ YN0028: The lockfile would have been modified by this install, which is explicitly forbidden.
```

**The drift is format-only and contains no dependency changes.** Diffed in full: 10 lines,
being `__metadata: version 8 → 10` plus the `resolve` builtin-patch `hash=`/`checksum`
values. Not one package version moves. The committed lock was generated by an older
Yarn 4.x than the 4.18 used here.

There is **no `packageManager` field in `package.json` and no `yarnPath` in `.yarnrc.yml`**,
so nothing pins the Yarn version — whatever the build image ships is what runs, and a
mismatch fails the build. Fix before the deployment workflow (migration plan step 5), by
either route:

- add `"packageManager": "yarn@<version>"` matching whatever generated the lock, or
- pick the Yarn version deliberately, regenerate the lock once with it, and commit both.
...
5. ~~Add `html2canvas`; confirm `tsc --noEmit` is green and `next build` succeeds
   locally.~~ **Done 2026-08-23** (§0). Still outstanding in this step: pin the Yarn
   version so `--immutable` passes, and take the Next.js 14.2.x patch bump.
...
8. **Pin the Yarn version, or regenerate the lockfile?** `--immutable` currently fails on
   a format-only drift with no dependency changes. Either add `packageManager` matching the
   lock, or regenerate the lock against a chosen Yarn version. Needed before CI. See §0.
9. **Which account holds the existing `nightingale.im` zone?** Not verifiable from here —
