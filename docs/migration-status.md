...
The blocker described here is closed.

The recovered manifest is **identical to the Jul-20 reconstruction except for one line** —
`"html2canvas": "^1.4.1"` — which is exactly the drift predicted in §1. Nothing else moved
in a month of development.

### Verified end-to-end after the commit ✅

Run against a clean scratchpad checkout of the tracked repo, Yarn 4.18 (Berry — the
lockfile is `__metadata: version 8`, so Yarn 1 cannot read it):

| Step | Result |
|---|---|
| `yarn install` | ✅ (after the `.yarnrc.yml` fix below) |
| `yarn prisma generate` | ✅ `Generated Prisma Client (v6.7.0) to ./node_modules/@prisma/client` |
| `tsc --noEmit` | ✅ **zero errors** — the `html2canvas` failure is gone |
| `next build` | ✅ completes; all 16 API routes and 12 pages compile, middleware 26.4 kB, shared JS 87.2 kB |

**The repo now builds.** That was the last thing standing between it and a deployable state.

### Fixed in this session: `.yarnrc.yml` had a dead Abacus VM path

`globalFolder: /opt/hostedapp/node/yarn/global` — flagged as suspicious earlier, now
**proven fatal**, not cosmetic:
...
`yarn install` fails outright on any machine that is not the Abacus VM — this Mac, a
Vercel build image, a GitHub Actions runner. Same family as the hardcoded Prisma `output`
path removed on 2026-08-23. The line is deleted; install succeeds without it.

### ~~Open: the Yarn version is unpinned~~ RESOLVED — pinned to `yarn@4.18.0`

**Rob, 2026-08-23: regenerate.** Done.

- `package.json` gains `"packageManager": "yarn@4.18.0"` — a single added line.
- `yarn.lock` regenerated against that exact version.

**The regenerated lock changes 5 lines and no package versions.** Full diff, verified
before committing:

| Line | Change |
|---|---|
| `__metadata: version` | `8` → `10` (lockfile format) |
| `resolve@patch` ×2 | `hash=c3c19d` → `hash=9bd1a5` (builtin `compat/resolve` patch) |
| `resolve` checksums ×2 | recomputed for the above |

Nothing else. No dependency was added, removed, or moved — the drift was purely the
lockfile format emitted by a newer Yarn, exactly as measured beforehand.

**Re-verified after regeneration, on a clean checkout:**

| Step | Result |
|---|---|
| `yarn install --immutable` | ✅ **passes** — this was the failing case |
| `yarn prisma generate` | ✅ Prisma Client 6.7.0 |
| `tsc --noEmit` | ✅ exit 0, zero errors |
| `next build` | ✅ `Compiled successfully` |

CI can now run `yarn install --immutable` and get a reproducible tree. Note that
`packageManager` is honoured by Corepack, which is enabled by default on Vercel and on
`actions/setup-node` — so the pin takes effect on the hosts that matter without extra
config. It is **not** installed on this Mac (no `corepack` binary), so local work needs
either `npm i -g corepack` or an explicit Yarn 4.18.0 invocation.
...
> **RESOLVED — `html2canvas@^1.4.1` is in the committed manifest (`f448398`), and
> `tsc --noEmit` now reports zero errors. See §0.**

**Conclusion (as of the Jul-20 reconstruction): the code typechecks cleanly apart from one
missing dependency, `html2canvas`.** With that added, `tsc` is green. This matters because
`next.config.js` sets `typescript.ignoreBuildErrors = false` — a TS error fails the build.

### Other build-environment landmines found

- ~~**`.yarnrc.yml` points at an Abacus VM path.**~~ **FIXED this session** — it was not
  merely wrong, it made `yarn install` fail with `EACCES … mkdir '/opt/hostedapp'` on any
  non-Abacus machine. Line removed; install verified. See §0.
- ⚠️ **The Yarn version is unpinned and `--immutable` fails.** Blocks CI as written.
  Details and the two fix options in §0.
...
5. ~~Add `html2canvas`; confirm `tsc --noEmit` is green and `next build` succeeds
   locally. Pin the Yarn version so `--immutable` passes.~~ **Done 2026-08-23** (§0).
   Still outstanding in this step: the Next.js 14.2.x patch bump.
...
7. ~~Pin the Yarn version, or regenerate the lockfile?~~ **Regenerate — done.** Pinned to
   `yarn@4.18.0`; `--immutable` passes; no package versions changed. See §0.

**Still open:**

9. **Email Routing for the six inbound mailboxes** — needs setting up alongside sending,
   or the forms deliver into a void. See §2.
10. **Which account holds the existing `nightingale.im` zone?** Not verifiable from here —
